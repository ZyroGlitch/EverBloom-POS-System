...
function Sales({ employees, order_id, orders }) {
    ...
    function submit(e) {
        e.preventDefault();
        post(route('admin.searchEmployee'), {
            onSuccess() {
                reset();
            }
        })
                ...
                <div className="col-md-3 d-flex flex-column gap-3">
...

                    {
                        employees.map((employee) => (
                            <Link
                                className="card shadow-sm rounded-lg border-0 bg-light"
                                style={{ textDecoration: 'none' }}
                                key={employee.id}
                            >
                                <div className="card-body d-flex align-items-center gap-3">
                                    <img
                                        src={`/storage/${employee.profile}`}
                                        alt="profile"
                                        className="object-fit-cover rounded-circle border border-2 border-primary shadow-sm"
                                        style={{ width: '50px', height: '50px', padding: '1.5px' }}
                                    />

                                    <div>
                                        <p className='fw-bold'>{employee.firstname} {employee.lastname}</p>
                                        <p className="text-success">{employee.role}</p>
                                    </div>
                                </div>
                            </Link>
                        ))
                                            ...
                                            order_id ? (
                                                orders.data.map((order, index) => (
                                                    <tr className='align-middle' key={index}>
                                                        ...
                                                        <td>
                                                            <Link
                                                                href={route('admin.sales', { order_id: order_id[index]?.order_id })}
                                                                className='text-dark'
                                                            >
                                                                <FaEye />
                                                            </Link>
